...
def get_hflip(pixel_values, model):
    pass
    # pixel_values = 
    # return 

def get_vflip(pixel_values, model):
    pass


def get_hvflip(pixel_values, model):
    pass

def get_flip(pixel_values, model):

    hflip_image = pixel_values.flip(-1)
    vflip_image = pixel_values.flip(-2)
    vhflip_image = pixel_values.flip((-1, -2))
    
    hflip_logit = model(pixel_values=hflip_image).logits
    dehflip_logit = hflip_logit.flip(-1)
    
    vflip_logit = model(pixel_values=vflip_image).logits
    devflip_logit = vflip_logit.flip(-2)
    
    vhflip_logit = model(pixel_values=vhflip_image).logits
    devhflip_logit = vhflip_logit.flip((-1, -2))
    
    return (dehflip_logit+devflip_logit+devhflip_logit) / 3
    
    
    
    
def get_rotate90(pixel_values, model):
    logits = []
    # print("SHAPE", pixel_values.shape)
    for i in range(1, 4):
        rotated_image = torch.rot90(pixel_values, i, (2, 3))
        # print("SHAPE_ROT", rotated_image.shape)
        rotated_logits = model(pixel_values=rotated_image).logits
        unrotated_logits = torch.rot90(rotated_logits, -i, (2, 3))
        logits.append(unrotated_logits)
...
def simple_inference(config: Config, model, scale):
    ...
    for image_path in tqdm(list(test_dir.glob("*"))):
        ...
        with torch.no_grad():
            ...
            # config.dataset.input_size = 1024
            ...
            if condition:
                ...
                original_logits = model(pixel_values=pixel_values).logits
                
                rotated_logits = get_rotate90(pixel_values, model)
                flipped_logits = get_flip(pixel_values, model)
                
                mean_logits = (original_logits+rotated_logits+flipped_logits) / 3
